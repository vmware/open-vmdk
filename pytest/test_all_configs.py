...
def test_product_sections_configs():
    #ProductSection
    ...
    for i in range(len(cfg_product_sections)):
        for el1, el2 in key_dict_product.items():
            assert_values(cfg_product_sections[i].get(el1), cfg_vmw_ovf[i].get(el2))

        if cfg_product_sections[i].get('properties'):
            ...
            for key in cfg_product_sections[i]['properties']:
                 ...
                 for el1, el2 in key_dict_properties.items():
                     assert_values(cfg_product_sections[i]['properties'][key].get(el1), properties[j].get(el2))
                 assert_values(cfg_product_sections[i]['properties'][key].get('user_configurable'),  bool(properties[j].get('@ovf:userConfigurable')))
                 assert_values(cfg_product_sections[i]['properties'][key].get('password'),  bool(properties[j].get('@ovf:password')))

        if 'categories' in cfg_product_sections[i]:
            assert_values(list(cfg_product_sections[i]['categories'].values()), list(cfg_vmw_ovf[i].get('Category')))

        if 'transports' in cfg_product_sections[i]:
            transports.extend(cfg_product_sections[i]['transports'])

    if transports:
        assert_values(set(transports), set(ovf['Envelope']['VirtualSystem']['VirtualHardwareSection']['@ovf:transport'].split()))

def test_annotation_configs():
    #Annotation Section
    config, ovf = get_configs()
